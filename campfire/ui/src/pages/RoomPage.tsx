...
export default function RoomPage() {
  ...
  const { currentRoom, joinRoom, joinHostedRoom, leaveRoom } = useRoom();
  const {
    peers,
    localStream,
    messages,
  ...
  const doJoin = async () => {
    if (!name || !hostClean || joining) return;
    setJoining(true);
    setPromptJoin(false);
    try {
      if (isHost) {
    ...
    return (
      <div className="h-screen bg-stone-950 text-stone-100 flex items-center justify-center">
        <div className="flex flex-col items-center gap-6 max-w-sm mx-4">
          ...
          <p className="text-stone-400 text-sm text-center font-mono">
            ~{hostClean}/{name}
          </p>
          <button
            onClick={doJoin}
            className="bg-amber-600 hover:bg-amber-500 text-stone-950 font-medium px-8 py-3 rounded-lg transition-colors"
          >
            Join
          </button>
          <button onClick={() => navigate("/")} className="text-stone-500 hover:text-stone-300 text-sm">
            Back
          </button>
        </div>
      </div>
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(
                `${window.location.origin}/apps/campfire/room/~${hostClean}/${name}`
              );
            }}
            className="text-xs text-stone-500 hover:text-amber-500 transition-colors px-2 py-1"
          >
            Copy Link
          </button>
        ...
        {showChat && (
          <div className="w-72 bg-stone-900 border-l border-stone-800/50 flex flex-col shrink-0">
            <div className="px-4 py-3 border-b border-stone-800/50">
              ...
              <p className="text-[10px] text-stone-600 mt-0.5">Ephemeral — gone when you leave</p>
            </div>
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex flex-col ${m.speaker === "me" ? "items-end" : "items-start"}`}>
                  <span className="text-[10px] text-stone-600 font-mono">
                    {m.speaker === "me" ? `~${ship}` : `~${m.speaker}`}
                  </span>
                  <div className={`px-2.5 py-1 rounded-lg text-sm max-w-[85%] break-words ${m.speaker === "me" ? "bg-amber-700/30 text-amber-100" : "bg-stone-800 text-stone-200"}`}>
                    {m.text}
                  </div>
                </div>
              ))}
